	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for (int i = 0; i < prenotazioni.size(); i++) {
			if (p.getIdcamera() == prenotazioni.get(i).getIdcamera()) {
				if ((((p.getCheckin().compareTo(prenotazioni.get(i).getCheckout()) > 0)
						&& (p.getCheckout().compareTo(prenotazioni.get(i).getCheckout()) > 0))
						|| ((p.getCheckin().compareTo(prenotazioni.get(i).getCheckin()) < 0)
								&& (p.getCheckout().compareTo(prenotazioni.get(i).getCheckin()) < 0)))
						&& ((p.getCheckin().compareTo(prenotazioni.get(i).getCheckin()) > 0)
								&& (p.getCheckout().compareTo(prenotazioni.get(i).getCheckin()) < 0))) {
